...
const candidates = process.platform === 'darwin'
  ? [path.join(release, 'mac-arm64', 'OAT.app')]
...
await new Promise((resolve, reject) => {
  // macOS 26+ aborts GUI applications started by executing Contents/MacOS
  // directly outside LaunchServices. `open` keeps the smoke headless while
  // giving Electron a valid application registration context.
  const command = process.platform === 'darwin' ? 'open' : executable;
  const args = process.platform === 'darwin'
    ? ['-W', '-n', '--env', 'OAT_ZVEC_COMPATIBILITY_SMOKE=1', '--env', `OAT_ZVEC_COMPATIBILITY_REPORT=${report}`, executable]
    : [];
  ...
  child.once('exit', (code, signal) => { clearTimeout(timer); code === 0 ? resolve() : reject(new Error(`Packaged Zvec smoke exited with ${code ?? signal}.`)); });
});
