...
contextBridge.exposeInMainWorld('oatDesktop', {
  ...
  sendResourceAgentMessage: (text: string, requestId?: string) => ipcRenderer.invoke('resource-agent:send', requestId ? { text, requestId } : text),
  onResourceAgentEvent: (listener: (payload: { requestId: string; event: Record<string, unknown> }) => void) => { const handler = (_event: Electron.IpcRendererEvent, payload: { requestId: string; event: Record<string, unknown> }) => listener(payload); ipcRenderer.on('resource-agent:event', handler); return () => ipcRenderer.removeListener('resource-agent:event', handler); },
