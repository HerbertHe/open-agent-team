...
function AgentTreeRow({ member, selected, thinking, nested = false, onSelect }: { member: Agent; selected: boolean; thinking: boolean; nested?: boolean; onSelect(): void }) {
  ...
  return <button type="button" onClick={onSelect} className={`${nested ? 'w-full' : 'ml-2 w-[calc(100%-0.5rem)]'} mt-1 flex items-center gap-1.5 rounded-md border px-2 py-1.5 text-left text-body leading-5 ${classicTone}`}>
    {thinking ? <IconLoader className="h-3 w-3 shrink-0 animate-spin text-oat-taupe" /> : <i className={`h-1.5 w-1.5 shrink-0 rounded-full ${isAdmin ? 'bg-amber-500' : member.status === 'running' ? 'bg-emerald-500' : member.status === 'failed' ? 'bg-red-500' : 'bg-amber-500'}`} />}
    ...
    <ol>{reports.map((report) => {
      ...
      return <li key={report.id} className={isLeader ? 'is-leader' : 'is-worker'}>
        ...
        <div className="delivery-report-content">
          <header><strong>{agentName(report.agentId)}</strong><span>→</span><b>{isLeader ? t('chat.adminRole') : t('chat.leaderRole')}</b><time>{new Date(report.createdAt).toLocaleString()}</time></header>
