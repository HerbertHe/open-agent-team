...
本项目提供一个“声明式的 agent team 构建”流程：你在 `team.yaml` 中声明 `Admin / Leader / Worker` 的角色、模型、skills、以及团队分支/工作空间策略；运行时由 Orchestrator 读取配置并完成以下工作：
...
```mermaid
flowchart TD
  U[用户] --> CLI[oat start team.yaml "<goal>" --port PORT]
