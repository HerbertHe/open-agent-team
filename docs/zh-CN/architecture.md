...
- `team.worker.total`：worker 池大小（team 启动时预先创建并常驻；仅在 orchestrator 退出时由 `stopAll` 统一销毁）
- `team.worker.lifecycle` / `team.worker.skill_sync`：虽然在 schema/loader 中存在默认值，但当前实现暂未按该字段分支；leader 完成后不会再提前清理 worker 池
