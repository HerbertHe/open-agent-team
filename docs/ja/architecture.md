...
このプロジェクトは「宣言的な agent チーム」ワークフローを提供します。`team.yaml` 内で `Admin / Leader / Worker` のロール、モデル、skills、そしてチームごとのブランチ/ワークスペース戦略を宣言します。実行時には Orchestrator が設定を読み取り、以下を行います：
...
```mermaid
flowchart TD
  U[ユーザー] --> CLI[oat start team.yaml "<goal>" --port PORT]
