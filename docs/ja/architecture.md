...
- `team.worker.total`：worker プールサイズ。team 起動時に worker を事前作成し、停止/削除は orchestrator の終了時（`stopAll`）のみ
- `team.worker.lifecycle` / `team.worker.skill_sync`：デフォルト値はあるが、現状の実装はこれらの項目に完全には分岐していない（leader 完了時に worker プールをクリーンアップしない）
