...
const messages: Record<Lang, Record<MessageKey, string>> = {
  en: {
    ...
    scheduler_release_waiting_approval: "Team {teamName} completed implementation and Leader review and submitted release {proposalId}. Waiting for the Admin's final merge; the Leader's result is already authoritative.",
  ...
  "zh-CN": {
    ...
    scheduler_release_waiting_approval: "团队「{teamName}」已完成实现和 Leader 审核，并提交交付 {proposalId}。正在等待 Admin 执行最终合并；Leader 结论已生效，无需二次审批。",
  ...
  fr: {
    ...
    scheduler_release_waiting_approval: "L'équipe {teamName} a terminé l'implémentation et la revue du Leader et a soumis la livraison {proposalId}. En attente de la fusion finale par l'Admin ; la décision du Leader fait autorité.",
  ...
  ja: {
    ...
    scheduler_release_waiting_approval: "チーム「{teamName}」は実装と Leader レビューを完了し、リリース {proposalId} を提出しました。Leader の判断を確定結果として Admin の最終マージを待っています。",
